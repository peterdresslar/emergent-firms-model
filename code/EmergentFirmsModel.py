...
agentHistoryDF.to_csv(directory + experiment + '.csv', index=False, float_format=lambda x: f'{x:.10f}'.rstrip('0').rstrip('.'))

# Format node attributes before writing GML
for node in F.nodes():
    for attr in ['savings', 'wage', 'loan']:
        if attr in F.nodes[node]:
            F.nodes[node][attr] = f'{float(F.nodes[node][attr]):.10f}'.rstrip('0').rstrip('.')
